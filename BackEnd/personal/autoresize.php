...
if($check_user){
    if($db->Update('users', $check_user[0]["ID"], ["TOKEN" => $token, "LAST_AUTH" => time()]) == $check_user[0]["ID"]){
        echo json_encode(password_verify($password, $check_user[0]["PASSWORD"]) ? ["ID" => $check_user[0]["ID"], "LOGIN" => $login ,"TOKEN" => $token, "NAME" => $check_user[0]["NAME"] . " " . $check_user[0]["LAST_NAME"], "IS_ADMIN" => $check_user[0]["ID"]] : ["ERROR" => "Не правильный пароль"]);
    }else{
